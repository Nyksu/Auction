...
if(!isFirst){
	if (TextFormData(Request.Form("is_del"),"")=="1"){
		...
		if (!/(\d{1,2}).(\d{1,2}).(\d{4})$/.test(d_end)){ErrorMsg+="'Дата окончания' некорректная.<br>"}
		if (ErrorMsg=="") {
			res=sumdat.DateComparing(d_end,end_date)
			if (res==null) {ErrorMsg+="'Дата окончания' некорректная.<br>"}
			else {
				if (res<=0) {ErrorMsg+="Дата продления должна быть больше даты окончания аукциона.<br>"}
			}
		}
		if (ErrorMsg=="") {
			sql=set_lot_prolong
...
<html>
<head>
<title>Сибирский Аукцион полезных вещей. Продление лота.</title>
...
<form name="form1" method="post" action="prolong_lot.asp">
  ...
  <table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#333333">
    <tr bgcolor="#FFCC99" bordercolor="#FFCC99"> 
      ...
      <td colspan="2"> 
        <p><b><a href="auction.asp"><font face="Verdana, Arial, Helvetica, sans-serif">Весь 
          КАТАЛОГ</font></a> &gt;&gt; <a href="auction.asp?subj_id=<%=subj%>"><%=subj_name%></a> 
          &gt;&gt; <%=name_lot%> </b></p>
      </td>
    </tr>
  </table>
    ...
    <tr valign="top"> 
      ...
      <td colspan="2" bgcolor="#FFFFFF" bordercolor="#FFFFFF" height="148"> 
        <table border="0" cellspacing="0" cellpadding="0" width="100%">
            ...
            <td bgcolor="#EBF3F2" colspan="2"> 
              <div align="center"> 
                <p align="left"><font size="1"><b><img src="<%="Images/"+nam_img+".gif"%>" height="16" align="absmiddle" alt="<%=tip_nam%>"></b> 
                  (лот № <%=lot%>)</font> <a href="auction.asp?subj_id=<%=subj%>&lot_id=<%=lot%>"><%=name_lot%></a></p>
              </div>
            </td>
          ...
          <tr> 
            <td bgcolor="#FFCC99" colspan="2" height="24"> 
              <p><b>Текущая цена лота:</b><font color="#3333FF"> <b><font color="#FF0000"><%=tek_price%></font> 
                <%=valuta%> </b></font><%=kvo_stavok==0?" Ставок нет!":" Уже сделано "+kvo_stavok+" ставок."%></p>
            </td>
          </tr>
          <tr> 
          ...
          <tr> 
            <td> 
              <p>Начало торгов: <%=start_date%></p>
            </td>
            <td> 
              <p>Окончание торгов: <%=end_date%></p>
            </td>
          </tr>
          <tr>
            <td bgcolor="#EBF3F2"> 
              <p align="right"><b><font size="2" color="#FF0000">Новая дата окончания:</font></b></p>
            </td>
          ...
          <tr> 
            <td> 
              <p align="right"><b>Ваше согласие:</b></p>
            </td>
            <td> 
              <p align="left"> <font color="#3333FF"> 
                <input type="checkbox" name="is_del" value="1">
                <font size="3" color="#CC0000"><b>Да, я действительно хочу продлить 
                лот! 
                <input type="submit" name="Submit" value="Продлить Лот">
                </b></font></font></p>
            </td>
          </tr>
        </table>
...
<div align="right">
  <div align="center">
    <p><font size="3" color="#0000FF"><b>Лот продлен. </b></font></p>
  </div>
  <p align="center">&nbsp; </p>
  <p align="center"> 
    <%}%>
...
<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#330000">
  <tr bgcolor="#FFCC99" bordercolor="#FFCC99"> 
    ...
    <td colspan="2"> 
      <p><b><a href="auction.asp"><font face="Verdana, Arial, Helvetica, sans-serif">Весь 
        КАТАЛОГ</font></a> &gt;&gt; <font color="#6699FF"><a href="auction.asp?subj_id=<%=subj%>"> 
